...
\begin{equation}
LPIPS(x,\hat{x}) = \sum_{l=1}^{L} \frac{1}{H_lW_l}\sum_{h=1}^{H_l}\sum_{w=1}^{W_l} \left\| \alpha_l \odot \left(\hat{f}_l(x)_{h,w} - \hat{f}_l(\hat{x})_{h,w}\right) \right\|_2^2,
\end{equation}
where $\hat{f}_l(\cdot)$ is the unit-normalized feature map extracted from layer $l$ of a pretrained network, $(H_l,W_l)$ are the spatial dimensions of that layer, $\alpha_l$ are learned per-channel weights, and $\odot$ denotes element-wise multiplication. Lower LPIPS values indicate that the reconstructed image is closer to the original in terms of perceptually important structures and textures, making it especially relevant for GAN-based compression where visual quality is prioritized.

...
\begin{figure}[htbp]
    ...
    \caption{General structure of a convolutional neural network \cite{developersbreachCNN}}
...
While the encoder reduces spatial resolution, the generator must increase resolution to reconstruct the output image. This is achieved by up-sampling operations, most commonly transposed convolution (also called deconvolution) or nearest-neighbor up-sampling followed by convolution. Transposed convolution can be understood as the inverse spatial operation of a normal convolution: it expands the feature map and then applies a learnable kernel to fill in the higher-resolution detail. In the HiFiC generator, several up-convolution blocks progressively restore the spatial resolution of the latent representation back to the original image size.
...
\begin{figure}[H]
    ...
    \caption{HLS compilation, scheduling, binding, and RTL generation flow \cite{amd2026vitisHls}}
...
\begin{figure}[H]
    ...
    \caption{Scheduling and binding example \cite{amd2026vitisHls}}
...
\begin{figure}[H]
    ...
    \caption{Loop pipeline timing example \cite{amd2026vitisHls}}
...
\begin{figure}[H]
    ...
    \caption{Task-level dataflow timing example \cite{amd2026vitisHls}}

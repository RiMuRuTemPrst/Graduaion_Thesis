...
Figure~\ref{fig:flow_design} shows the overall implementation flow used in this thesis. The flow connects the software model, HLS accelerator design, FPGA system integration, and board-level execution into a single development process.
...
This section describes the internal architecture of the proposed accelerator. The entire HiFiC generator is implemented as a single HLS IP, \texttt{full\_generator\_top}, organized into three stages that share the same arithmetic style: the \textbf{Fusion Core} (initial convolution block, nine residual blocks, and the global skip addition), the \textbf{UpConv Core} (four transposed-convolution up-sampling blocks), and the \textbf{Conv77 Core} (the final $7\times7$ convolution that produces the RGB image). Packing the whole generator into one IP allows the multiply--accumulate (MAC) engines to be reused across stages and simplifies system integration, at the cost of a tight on-chip resource budget. The explanation below starts from the common building blocks -- data buffering and the processing element -- then describes the overall architecture and each of the three stages.
...
At a conceptual level, the accelerator is built from two reusable functional units, as shown in Figure~\ref{fig:gan_accelerator_concept}: a \textbf{Convolution Functional Unit (Conv FU)} that contains the MAC array (the PE cluster described above), and a \textbf{Normalization/Activation Functional Unit (Norm/Act FU)} that performs channel normalization and the activation function. Both units exchange data with external DDR memory through AXI4 master interfaces. Every stage of the generator -- Fusion, UpConv, and Conv77 -- reuses this same convolution-then-normalization motif, which is why a small number of hardware engines can implement the whole network.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{Images/gan_accelerator_concept.png}
    \caption{Conceptual architecture of the generator accelerator: a convolution functional unit (MAC array) and a normalization/activation functional unit sharing DDR access through AXI}
...
\begin{figure}[H]
    ...
    \caption{Convolution-then-normalization datapath shared by the generator stages, shown for a residual block}
...
\subsubsection{Fusion Core (Stage 1)}
...
A single shared engine implements all of these blocks. Because the CBI, the nine residual blocks, and the global add all operate on the same $16\times16\times960$ tensor, they are mapped to one hardware instance that is invoked repeatedly with different parameters, rather than replicated. To keep this as a single instance, every call passes the same set of ports; if some calls passed null pointers and others did not, the tool would infer two different instances and exceed the on-chip memory budget. The engine uses sixteen PEs (128 MAC/cycle) with the rotating accumulator, and stores two large skip tensors in UltraRAM (URAM): a \emph{global skip buffer} that holds the CBI output until the global add, and a \emph{skip-connection buffer} that holds the input of each residual block until its residual add.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Images/fusion_core.png}
...
\subsubsection{UpConv Core (Stage 2)}

The UpConv Core performs the four up-sampling blocks. Each block is a $3\times3$ transposed convolution with stride two that doubles the spatial resolution and halves the channel count, as summarized in Table~\ref{tab:upconv_steps}.
...
The transposed convolution is implemented by a \emph{scatter-and-accumulate} scheme: each input pixel distributes its MAC contributions to several output positions according to the $3\times3$ kernel and the stride-two pattern, accumulating into a row accumulator (\texttt{row\_acc}) stored in URAM and split into 32 banks for parallel access. The core uses eight PEs ($\text{PEs}_U = 8$, eight output channels in parallel) and applies a per-pixel normalization, in which the mean and variance are computed across channels at each output pixel before normalization and ReLU. As in the Fusion Core, a single shared engine serves all four blocks; the input source is selected by an internal mode parameter so that the tool infers only one hardware instance. Figure~\ref{fig:upconv_timing} shows the timing path of the up-convolution engine, in which the loading of input rows is overlapped with the computation of output rows.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Images/upconv_timing.png}
    \caption{Timing path of the up-convolution engine, overlapping input-row loading with output-row computation}
    \label{fig:upconv_timing}
\end{figure}

\subsubsection{Conv77 Core (Stage 3)}

The Conv77 Core implements the final output block: reflection padding, a $7\times7$ convolution that maps the 60 feature channels to the 3 RGB channels, and a clipping operation. It combines two forms of parallelism: eight input channels are processed in parallel ($\text{SIMD\_DEPTH}=8$) and eight output windows are computed in parallel ($\text{NUM\_WIN\_PEs}=8$), which together with the $7\times7$ kernel reaches up to 448 MAC operations per cycle. Unlike the other two cores, the Conv77 Core uses only Block RAM (no URAM) for its small input-window and weight buffers. A final clipping to the range $[0,1]$ is applied, as required by the HiFiC specification for normalized images.
...
\hline
Skip-connection buffer & $16\times16\times60$ & URAM & residual skip of each res\_block \\
\hline
Global skip buffer & $16\times16\times60$ & URAM & CBI output for the global add \\
\hline
